...
function name(value: unknown, fallback = '—'): string { const item = row(value); return text(item.topicName ?? item.subjectName ?? item.name ?? item.label ?? item.title ?? item.category, fallback); }
...
export function AnalyticsScreen({ navigation }: MoreStackScreenProps<'Analytics'>): React.JSX.Element {
    ...
    const forecast = data?.improvementForecast;

    const renderSummary = (): React.JSX.Element => <><View style={styles.summaryRow}><View style={styles.summaryCard}><Text style={styles.summaryLabel}>{t('analytics.latestScore')}</Text><Text style={styles.summaryValue}>{scoreSummary.latest ? `${Math.round(scoreSummary.latest)}%` : '—'}</Text></View><View style={styles.summaryCard}><Text style={styles.summaryLabel}>{t('analytics.averageScore')}</Text><Text style={styles.summaryValue}>{scoreSummary.average ? `${Math.round(scoreSummary.average)}%` : '—'}</Text></View><View style={styles.summaryCard}><Text style={styles.summaryLabel}>{t('analytics.bestScore')}</Text><Text style={styles.summaryValue}>{scoreSummary.best ? `${Math.round(scoreSummary.best)}%` : '—'}</Text></View></View><View style={styles.card}><Text style={styles.heading}>{t('analytics.scoreTrajectory')}</Text>{points.length ? points.slice(-5).map((item, index) => <Bar key={index} label={dateLabel(row(item).date ?? row(item).createdAt) || name(item, t('analyticsScreen.insight'))} value={metric(item)} caption={`${Math.round(metric(item))}%`} selected={selection?.kind === 'score' && selection.value === item} color="#16a34a" onPress={() => setSelection({ kind: 'score', value: item })} />) : <Text style={styles.muted}>{t('analytics.emptyScores')}</Text>}</View><View style={styles.card}><Text style={styles.heading}>{t('analytics.weakAreas')}</Text>{weakAreas.slice(0, 5).map((item, index) => <Bar key={index} label={name(item, t('analyticsScreen.insight'))} value={metric(item)} selected={selection?.kind === 'weak' && selection.value === item} onPress={() => setSelection({ kind: 'weak', value: item })} />)}{weakAreas.length === 0 ? <Text style={styles.muted}>{t('analytics.emptyWeak')}</Text> : null}</View></>;
    const renderWeakAreas = (): React.JSX.Element => <View style={styles.card}><Text style={styles.heading}>{t('analytics.weakAreas')}</Text><Text style={styles.muted}>{t('analytics.tapToDrillDown')}</Text>{weakAreas.map((item, index) => <Bar key={index} label={name(item, t('analyticsScreen.insight'))} value={metric(item)} selected={selection?.kind === 'weak' && selection.value === item} onPress={() => setSelection({ kind: 'weak', value: item })} />)}{weakAreas.length === 0 ? <Text style={styles.muted}>{t('analytics.emptyWeak')}</Text> : null}</View>;
    const renderScores = (): React.JSX.Element => <View style={styles.card}><Text style={styles.heading}>{t('analytics.scoreTrajectory')}</Text><Text style={styles.muted}>{t('analytics.tapToDrillDown')}</Text>{points.length ? points.map((item, index) => <Bar key={index} label={`${dateLabel(row(item).date ?? row(item).createdAt) || name(item, t('analyticsScreen.insight'))}${row(item).source ? ` · ${text(row(item).source, '')}` : ''}`} value={metric(item)} caption={`${Math.round(metric(item))}%`} selected={selection?.kind === 'score' && selection.value === item} color="#16a34a" onPress={() => setSelection({ kind: 'score', value: item })} />) : <Text style={styles.muted}>{t('analytics.emptyScores')}</Text>}</View>;
    const renderTopics = (): React.JSX.Element => <View style={styles.card}><Text style={styles.heading}>{t('analytics.topicTrends')}</Text><Text style={styles.muted}>{t('analytics.tapToDrillDown')}</Text>{topics.map((item, index) => { const itemRow = row(item); const value = number(itemRow.avgQuestionsPerYear ?? itemRow.appearanceCount ?? itemRow.frequency) ?? 0; const max = Math.max(1, ...topics.map((topic) => number(row(topic).avgQuestionsPerYear ?? row(topic).appearanceCount ?? row(topic).frequency) ?? 0)); return <Bar key={index} label={name(item, t('analyticsScreen.insight'))} value={value / max * 100} caption={text(itemRow.appearanceCount ?? itemRow.avgQuestionsPerYear, t('analytics.review'))} selected={selection?.kind === 'topic' && selection.value === item} onPress={() => setSelection({ kind: 'topic', value: item })} color="#7c3aed" />; })}{topics.length === 0 ? <Text style={styles.muted}>{t('analytics.emptyTopics')}</Text> : null}</View>;
    ...
    const renderBenchmark = (): React.JSX.Element => <><View style={styles.card}><Text style={styles.heading}>{t('analytics.benchmark')}</Text>{data?.benchmark ? <><Text style={styles.bigMetric}>{text(row(data.benchmark).percentile ?? row(data.benchmark).estimatedPercentile)}%</Text><Text style={styles.detailText}>{t('analytics.percentile')} · {t('analytics.cohort')}: {text(row(data.benchmark).cohortSize ?? row(data.benchmark).sampleSize)}</Text><Text style={styles.detailText}>{t('analytics.minutes')}: {text(row(data.benchmark).userMinutes, '0')} · {t('analytics.median')}: {text(row(data.benchmark).cohortMedianMinutes, '0')}</Text></> : <Text style={styles.muted}>{t('analytics.noBenchmark')}</Text>}</View><View style={styles.forecastCard}><Text style={styles.forecastEyebrow}>{t('analyticsScreen.nextRange')}</Text><Text style={styles.heading}>{t('analytics.nextPracticeRange')}</Text>{forecast?.kind === 'ESTIMATE' ? <><Text style={styles.forecastMetric}>{forecast.estimatedNextMarks.low}–{forecast.estimatedNextMarks.high}</Text><Text style={styles.detailText}>{t('analytics.outOf')} {forecast.estimatedNextMarks.maximum} · {forecast.confidence.toLowerCase()} {t('analytics.confidence')}</Text><Text style={styles.detailText}>{forecast.message}</Text><View style={styles.assumptionBox}><Text style={styles.assumptionTitle}>{t('analyticsScreen.constructive')}</Text>{forecast.assumptions.slice(0, 2).map((assumption) => <Text key={assumption} style={styles.detailText}>• {assumption}</Text>)}</View><Text style={styles.muted}>{forecast.disclaimer}</Text></> : <Text style={styles.muted}>{forecast?.message ?? t('analytics.forecastNeedsHistory')}</Text>}</View></>;
    ...
    return <Screen title={t('analytics.title')}>
        {loading ? <ActivityIndicator color="#2563eb" /> : <ScrollView contentContainerStyle={styles.scroll}>
            ...
            <Pressable accessibilityRole="button" style={styles.drilldownButton} onPress={() => navigation.navigate('AnalyticsDrilldown')}><Text style={styles.drilldownText}>{t('analyticsScreen.openTrajectory')}</Text></Pressable>
